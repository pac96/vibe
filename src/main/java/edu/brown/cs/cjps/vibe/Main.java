...
public final class Main {
	...
	private void runSparkServer() {
		...
		Spark.post("/selectExistingPlaylist",
				new SelectExistingPlaylistHandler());
	}
	...
	private class LoginHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			 ...
			 * user
			 * 
			 * Vibe can read the user's email and modify public and private
			 * playlists
			 */
	...
	private class CodeHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			...
			if (!receivedCode) {
				 ...
				 * .getAsync method and synchronous requests are made with the
				 * .get method. This holds for all type of requests.
				 */
				...
				try {
					...
					currentUser = api.getMe().accessToken(accessToken).build()
							.get();
				} catch (Exception e) {
					e.printStackTrace();
			...
			try {
				events = eventProcessor
						.getEventsFromUserID(currentUser.getId());
			} catch (SQLException e) {
				e.printStackTrace();
				System.out.println("ERROR: Database issues");
			}

			Map<String, Object> frontEndInfo = ImmutableMap.of("username",
					display, "cachedEvents", events);
	...
	private class AddEventHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			...
			if (start.matches(TIMEREGEX) && end.matches(TIMEREGEX)) {
				...
				try {
					newEvent = eventProcessor.addEvent(start, amOrPm, end,
							endAmOrPm, eventName, currentUser.getId());
				} catch (SQLException e) {
	...
	private class GetPlaylistHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			...
			// Check to see if a prexisting playlist was associated with this
			// event
			...
			if (thisEvent.getPlayListURI().equals("")) {
				VibePlaylist playlist = VibeCache.getPlaylistCache().get(
						eventID);
				// TODO: Need to get the name from the eventID
				String eventName = thisEvent.getName();
				returnURI = hq.convertForSpotify(playlist, eventName, api,
						currentUser);
	...
	private class EditEventHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			...
			// (2): Make modifications to the event if the times match the
			// correct format
			if (start.matches(TIMEREGEX) && end.matches(TIMEREGEX)) {
				System.out.println("The input matches");
				CalendarEvent editedEvent = eventProcessor.editEvent(start,
						amOrPm, end, endAMOrPM, eventName, event);

				System.out.println("Edited event: " + editedEvent.toString());

				// (3): Generate the playlist associated with this event
				// hq.generateFromTag(editedEvent, api, currentUser,
				// accessToken);

				// (4): Return the event along to true so front end can
				// recognize that edit was succesful
				frontEndInfo = ImmutableMap.of("event", editedEvent, "success",
						true);
			} else {
				System.out.println("Incorrect time input");
				frontEndInfo = ImmutableMap
						.of("event", event, "success", false);
			}
			return GSON.toJson(frontEndInfo);
		}
	}
	...
	private class AddCustomPlaylistHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			...
			List<String> settingsList = Arrays.asList(tag, energy, hotness,
					mood);

			// Generate the playlist
			hq.generateCustom(genres, settingsList, thisEvent, api,
					currentUser, accessToken);
			System.out.println("returned from generatecustom");

			// These lines are only for testing
			VibePlaylist p2 = VibeCache.getPlaylistCache().get(
					thisEvent.getId());
	...
	private class GetAllPlaylistsHandler implements Route {
		...
		public Object handle(Request req, Response res) {
			...
			for (String[] playlist : playlistNames) {
				// TODO: something is wrong with this loop I think. It's only
				// going
